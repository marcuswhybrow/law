...
public class LawCommand implements CommandExecutor {
	...
	public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
		...
		if (command.getName().equalsIgnoreCase(PLUGIN_COMMAND_NAME)) {
			if (args.length <= 0) {
				...
				this.law.sendMessage(sender, "Unknown command. Type " + ChatColor.YELLOW + "/law" + ChatColor.WHITE + " for the list.");
			}
	...
	public void commandPrison(CommandSender sender, String[] args) {
		...
		if (args.length >= 2) {
			...
			} else if ("cell".equals(action)) {
				if (args.length >= 2) {
					if (args.length >= 3) {
						String cellAction = args[2];
						if ("create".equals(cellAction)) {
							if (args.length == 4) {
								...
								this.law.sendMessage(sender, ChatColor.RED + "usage: /" + PLUGIN_COMMAND_NAME + " prison cell create <cell-name>");
							}
						} else if ("delete".equals(cellAction)) {
							if (args.length == 4) {
								// Get the selected prison
								Prison selectedPrison = lawWorld.getSelectedPrison(player.getDisplayName());
								...
								this.law.sendMessage(sender, ChatColor.RED + "usage: /" + PLUGIN_COMMAND_NAME + " prison cell delete <cell-name>");
							}
						} else {
							// Not a cell action
							this.law.sendMessage(sender, "Unknown command. Type " + ChatColor.YELLOW + "/law prison cell" + ChatColor.WHITE + " for the list.");
						}
					} else {
						// List cell actions
						sender.sendMessage("/" + PLUGIN_COMMAND_NAME + " prison cell create <cell-name>");
						sender.sendMessage("/" + PLUGIN_COMMAND_NAME + " prison cell delete <cell-name>");
					}
				}
			} else {
				this.law.sendMessage(sender, "Unknown command. Type " + ChatColor.YELLOW + "/law prison" + ChatColor.WHITE + " for the list.");
			}
		} else {
			...
			sender.sendMessage("/" + PLUGIN_COMMAND_NAME + " prison cell ...");
		}
	}
}
